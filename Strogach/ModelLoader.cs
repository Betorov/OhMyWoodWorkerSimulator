...
namespace Strogach
{
    class ModelLoader
    {
        ...
        public ModelLoader(SharpDX.Direct3D11.Device device)
        {
            m_device = device;
            m_importer = new Assimp.AssimpContext();
            ...
            ref Matrix transform)
        {
            ...
            if (node.HasMeshes)
            {
                foreach (int index in node.MeshIndices)
                {
                    ...
                    var vertexBuffer = new SharpDX.Direct3D11.Buffer(device,
                                                    vertexStream,
                    ...
                    DataStream indexStream = new DataStream(indices.GetLength(0) * sizeof(int), true, true);
                    ...
                    var indexBuffer = new SharpDX.Direct3D11.Buffer(device,
                                                    indexStream,
                                                    new BufferDescription()
                                                    {
